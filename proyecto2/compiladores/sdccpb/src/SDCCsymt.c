    ...
    if (loop->bitVar) {
      ...
      if (loop->bitVar == BITVAR_PAD) {
        ...
        if (loop->bitVar <= (8 - bitOffset)) {
          ...
          if( (TARGET_IS_PIC16 || TARGET_IS_PICOBLAZE) && getenv("PIC16_PACKED_BITFIELDS") ) {
...
#if !OPT_DISABLE_PIC || !OPT_DISABLE_PIC16 || !OPT_DISABLE_PICOBLAZE
  /* The PIC port uses a different peep hole optimizer based on "pCode" */
  if (!TARGET_IS_PIC && !TARGET_IS_PIC16 && !TARGET_IS_PICOBLAZE)
#endif
...
initCSupport ()
{
  ...
  // PLAN: TODO PicoBlaze deals mul and div separatelly (in back-end) right now
  if (TARGET_IS_PICOBLAZE)
  {
	  return;
  }
