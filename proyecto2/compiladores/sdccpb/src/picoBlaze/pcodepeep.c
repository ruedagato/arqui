...
pCodeOp *picoBlaze_popCopyGPR2Bit(pCodeOpReg *pc, int bitval);
...
pCode * picoBlaze_findNextInstruction(pCode *pc);
int picoBlaze_getpCode(char *mnem,int dest);
int picoBlaze_getpCodePeepCommand(char *cmd);
void picoBlaze_pBlockMergeLabels(pBlock *pb);
char *picoBlaze_pCode2str(char *str, int size, pCode *pc);
//char *picoBlaze_get_op(pCodeOp *pcop,char *buf, size_t buf_size);
pCodeOp *picoBlaze_popCombine2(pCodeOp *, pCodeOp *, int);

extern pCodeInstruction *picoBlazeMnemonics[];
...
//*//      return PCOP(picoBlaze_popCopyGPR2Bit(&picoBlaze_pc_status,PIC_C_BIT));
//*//    if(toupper((unsigned char)*bit) == 'Z')
 //*//     return PCOP(picoBlaze_popCopyGPR2Bit(&picoBlaze_pc_status,PIC_Z_BIT));
  }

  // Check DC
//*//  if(len ==2 && toupper((unsigned char)bit[0]) == 'D' && toupper((unsigned char)bit[1]) == 'C')
//*//    return PCOP(picoBlaze_popCopyGPR2Bit(&picoBlaze_pc_status,PIC_DC_BIT));
...
static void * cvt_altpat_label(void *pp,pCodeWildBlock *pcwb)
{
  ...
  return picoBlaze_newpCodeLabel(NULL,-p->pct[1].tok.n);
...
static void * cvt_altpat_comment(void *pp,pCodeWildBlock *pcwb)
{
  ...
  return picoBlaze_newpCodeCharP(p->pct[0].tok.s);
...
static void * cvt_altpat_mnem0(void *pp,pCodeWildBlock *pcwb)
{
  ...
  opcode = picoBlaze_getpCode(p->pct[0].tok.s,0);
  ...
  if(opcode < 0) {
    ...
    opcode  = picoBlaze_getpCodePeepCommand(p->pct[0].tok.s);
    //if(opcode > 0)
    //  fprintf(stderr,"  but valid peep command: %s, key = %d\n",p->pct[0].tok.s,opcode);
    return NULL;
  }

  pci = PCI(picoBlaze_newpCode(opcode, NULL));
...
static void * cvt_altpat_mnem0a(void *pp, pCodeWildBlock *pcwb)
{
  ...
  return picoBlaze_newpCodeWild(p[0].pct[1].tok.n,NULL,NULL);
...
static void * cvt_altpat_mnem1(void *pp,pCodeWildBlock *pcwb)
{
  ...
  if(picoBlazeMnemonics[opcode]->isBitInst)
    pcosubtype = picoBlaze_newpCodeOp(p[1].pct[0].tok.s,PO_GPR_BIT);	//*//
  else {
//	fprintf(stderr, "%s:%d tok.s= %s\n", __FILE__, __LINE__, p[1].pct[0].tok.s);
    pcosubtype = picoBlaze_newpCodeOp(p[1].pct[0].tok.s,PO_GPR_REGISTER);	//GPR_REGISTER);
  }


  pci = PCI(picoBlaze_newpCode(opcode, pcosubtype));
...
static void * cvt_altpat_mnem1a(void *pp,pCodeWildBlock *pcwb)
{
  ...
  opcode = picoBlaze_getpCode(p->pct[0].tok.s,0);
  if(opcode < 0) {
    int cmd_id = picoBlaze_getpCodePeepCommand(p->pct[0].tok.s);
    pCode *pc=NULL;
    ...
    pc =  picoBlaze_newpCodeWild(p[1].pct[1].tok.n,NULL,NULL);
  ...
  if(picoBlazeMnemonics[opcode]->isBitInst)
    pcosubtype = picoBlaze_newpCodeOpBit(NULL,-1,0, PO_GPR_REGISTER);
  else
    pcosubtype = picoBlaze_newpCodeOp(NULL,PO_GPR_REGISTER);


  pci = PCI(picoBlaze_newpCode(opcode,
		     picoBlaze_newpCodeOpWild(p[1].pct[1].tok.n, pcwb, pcosubtype)));
...
static void * cvt_altpat_mnem1b(void *pp,pCodeWildBlock *pcwb)
{
  ...
  pci = PCI(picoBlaze_newpCode(opcode, picoBlaze_newpCodeOpLit(p[1].pct[0].tok.n) ));
...
static void * cvt_altpat_mnem2(void *pp,pCodeWildBlock *pcwb)
{
  ...
  opcode = picoBlaze_getpCode(p->pct[0].tok.s,dest);
  if(opcode < 0) {
    fprintf(stderr, "Bad mnemonic\n");
    return NULL;
  }
  ...
  if(picoBlazeMnemonics[opcode]->is2MemOp) {
  	/* support for movff instruction */
  	pcosubtype = picoBlaze_popCombine2(
  		picoBlaze_newpCodeOp(p[1].pct[0].tok.s, PO_GPR_REGISTER),
  		picoBlaze_newpCodeOp(p[3].pct[0].tok.s, PO_GPR_REGISTER), 0);
  } else
    pcosubtype = picoBlaze_newpCodeOp(p[1].pct[0].tok.s,PO_GPR_REGISTER);
...
static void * cvt_altpat_mnem2a(void *pp,pCodeWildBlock *pcwb)
{
  ...
  if(picoBlazeMnemonics[opcode]->isBitInst) {
    pcosubtype = picoBlaze_newpCodeOp(NULL,PO_GPR_BIT); //*//
    pcosubtype2 = NULL;
  } else {

    pcosubtype = picoBlaze_newpCodeOp(NULL,PO_GPR_REGISTER); pcosubtype2 = NULL; }
  ...
  if(!pcosubtype2)
  	...
  	pci = PCI(picoBlaze_newpCode(opcode,
		     picoBlaze_newpCodeOpWild2(p[1].pct[1].tok.n, p[3].pct[1].tok.n, pcwb, pcosubtype, pcosubtype2)));
...
static void * cvt_altpat_mnem2b(void *pp,pCodeWildBlock *pcwb)
{
  ...
  if(picoBlazeMnemonics[opcode]->is2MemOp) {
  	/* support for movff instruction */
	pcosubtype = picoBlaze_newpCodeOp(NULL, PO_GPR_REGISTER);
	pcosubtype2 = picoBlaze_newpCodeOp(NULL, PO_GPR_REGISTER);
  } else pcosubtype = pcosubtype2 = NULL;

  pci = PCI(picoBlaze_newpCode(opcode,
		     picoBlaze_newpCodeOpWild2(p[1].pct[1].tok.n, p[3].pct[1].tok.n,
...
static void * cvt_altpat_mnem3(void *pp,pCodeWildBlock *pcwb)
{
  ...
  if(picoBlazeMnemonics[opcode]->isBitInst) {
    ...
    //if(pcosubtype == NULL) {
    pcosubtype = picoBlaze_newpCodeOpBit(p[1].pct[0].tok.s,p[3].pct[0].tok.n,0, PO_GPR_REGISTER);
    //}
  } else
    pcosubtype = picoBlaze_newpCodeOp(p[1].pct[0].tok.s,PO_GPR_REGISTER);
...
static void * cvt_altpat_mnem4(void *pp, pCodeWildBlock *pcwb)
{
  ...
  if(picoBlazeMnemonics[opcode]->is2LitOp) {
    pcosubtype = picoBlaze_newpCodeOpLit2(p[1].pct[0].tok.n, picoBlaze_newpCodeOp(p[3].pct[0].tok.s, PO_GPR_REGISTER));
  }
...
static void * cvt_altpat_mnem4a(void *pp, pCodeWildBlock *pcwb)
{
  ...
  if(picoBlazeMnemonics[opcode]->is2LitOp) {
    pcosubtype = picoBlaze_newpCodeOpLit2(p[1].pct[0].tok.n, picoBlaze_newpCodeOpLit(p[3].pct[0].tok.n));
  }
	...
	if( j ) {
	  ...
	  case  PCP_STR:
	    ...
	    case PS_HAVE_MNEM:
	      ...
	      //pco1 = picoBlaze_newpCodeOp(NULL,PO_GPR_REGISTER);
...
//		dump1Token(tokArr[ltokIdx].tt);
	...
	if( altArr[k].f) {
	  ...
	  //if(curBlock && pc)
	  //picoBlaze_addpCode2pBlock(curBlock, pc);
	  if(pc) {
	    if (pcret) {
	      *pcret = pc;
	      return 0;       // Only accept one line for now.
	    } else
	      picoBlaze_addpCode2pBlock(pcwb->pb, pc);
	  } else
	    error++;
	}
	j += c;
...
/*-----------------------------------------------------------------*/
/* picoBlaze_AssembleLine - parse line and return the pCode equivalent */
/*                      peeps=1 if parsing peep rules, 0 otherwise */
/*-----------------------------------------------------------------*/
pCode *picoBlaze_AssembleLine(char *line, int peeps)
{
  pCode *pc=NULL;
...
static void initpCodePeep(pCodePeep *pcp)
{
  ...
  pcp->target.pb = picoBlaze_newpCodeChain(NULL, 'W', NULL);

  initpCodeWildBlock(&pcp->replace);
  pcp->replace.pb = picoBlaze_newpCodeChain(NULL, 'W', NULL);
...
/*                                                                 */
/*-----------------------------------------------------------------*/
extern void picoBlazeinitpCodePeepCommands(void);

void  picoBlaze_peepRules2pCode(peepRule *rules)
{
  peepRule *pr;
...
//	picoBlaze_printpBlock(stderr, currentRule->target.pb);

    //DFPRINTF((stderr,"target with labels merged:\n"));
    //picoBlaze_pBlockMergeLabels(curBlock);
    picoBlaze_pBlockMergeLabels(currentRule->target.pb);
    //picoBlaze_printpBlock(stderr, currentRule->replace.pb);

    //#ifdef PCODE_DEBUG
    //    picoBlaze_printpBlock(stderr, curBlock);
    //#endif
    //DFPRINTF((stderr,"\nReplaced by:\n"));
    ...
    picoBlaze_pBlockMergeLabels(currentRule->replace.pb);
    //picoBlaze_printpBlock(stderr, currentRule->replace.pb);
...

  {
    ...
    while(peeprules) {
      ...
      //picoBlaze_printpBlock(stderr, peepBlock->target.pb);
...
/*                                                                 */
/*-----------------------------------------------------------------*/
int picoBlaze_pCodeSearchCondition(pCode *pc, unsigned int cond)
{
...
static int pCodeOpCompare(pCodeOp *pcops, pCodeOp *pcopd)
{
  ...
  picoBlaze_get_op(pcops,b, sizeof(b) );

  n2 = picoBlaze_get_op(pcopd,NULL,0);
...
//	      return 1;
	  ...
	  /* assert that optimizations do not touch operations that work on SFRs or INDF registers */
	  if(PCOW2(PCI(pcd)->pcop) && (!(PCOP2(PCI(pcs)->pcop)->pcopR) || ((PCOP2(PCI(pcs)->pcop)->pcopR) ))) {
...
//		fprintf(stderr, "%s:%d %s second operand is wild\n", __FILE__, __LINE__, __FUNCTION__);
	  ...
	  } else if (PCOW2(PCI(pcd)->pcop) && PCOP2(PCI(pcs)->pcop)->pcopR)
	    {
	      return 0;
	    } else {
	      return havematch;
  ...
  if((pcd->type == PC_WILD) && (pcs->type == PC_OPCODE)) {
    ...
    pcs = picoBlaze_findNextInstruction(pcs->next); 
    if(pcs) {
...
/* Inserts p2 (1 item) after the head of p1 chain (1 or more items)*/
/* E.g.: p1 <-> p1next AND p2   splice into   p1 <-> p2 <-> p1next */
/*-----------------------------------------------------------------*/
void picoBlaze_pCodeInsertAfter(pCode *pc1, pCode *pc2)
{
  if(!pc1 || !pc2)
    return;
...
/*-----------------------------------------------------------------*/
/* picoBlaze_pCodeOpCopy - copy a pcode operator                       */
/*-----------------------------------------------------------------*/
...
static pCode *pCodeInstructionCopy(pCodeInstruction *pci,int invert)
{
  ...
  if(invert)
    new_pci = PCI(picoBlaze_newpCode(pci->inverted_op,pci->pcop));
  else
    new_pci = PCI(picoBlaze_newpCode(pci->op,pci->pcop));
...
/*
    ...
    if(IS_PCCOMMENT(pcin))
      pc = pcin = picoBlaze_findNextInstruction(pcin->next);
*/
    pcin = pc = picoBlaze_findNextInstruction(pc);
	...
	break;
      ...
      pcin = picoBlaze_findNextInstruction(pcin->next);
      pct = pct->next;
      //debug:
      //DFPRINTF((stderr,"    matched\n"));
...
//	fprintf(stderr," partial match... no more code\n");
    ...
    if(matched && pcin) {
	  ...
	  (picoBlaze_pCodeSearchCondition(pcin,peepBlock->postFalseCond) > 0) )
	matched = 0;

//	fprintf(stderr," condition results = %d\n",picoBlaze_pCodeSearchCondition(pcin,peepBlock->postFalseCond));
...
//	buf[0] = ';';
	...
	while(pc &&  pc!=pcin) {
	  ...
	  picoBlaze_pCode2str(&buf[1], sizeof( buf )-1, pc);
	  picoBlaze_pCodeInsertAfter(pcprev, picoBlaze_newpCodeCharP(buf));
	  pcprev = pcprev->next;
	  pc = pc->next;
...
//		fprintf(stderr, "%s:%d replacing index= %d\n", __FUNCTION__, __LINE__, index);
	      ...
	      if(peepBlock->target.wildpCodeOps[index])
		pcop = picoBlaze_pCodeOpCopy(peepBlock->target.wildpCodeOps[index]);
	      else
		DFPRINTF((stderr,"error, wildopcode in replace but not source?\n"));
	    } else
	      pcop = picoBlaze_pCodeOpCopy(PCI(pcr)->pcop);
	  }
...
//		fprintf(stderr, "%s:%d replacing index= %d\n", __FUNCTION__, __LINE__, index);
	      //DFPRINTF((stderr,"copying wildopcode\n"));
	      if(peepBlock->target.wildpCodeOps[index])
		pcop = picoBlaze_popCombine2(picoBlaze_pCodeOpCopy(pcop),
			picoBlaze_pCodeOpCopy(peepBlock->target.wildpCodeOps[index]), 0);
	      else
		DFPRINTF((stderr,"error, wildopcode in replace but not source?\n"));
	    } else
	      pcop = picoBlaze_popCombine2(picoBlaze_pCodeOpCopy(pcop),
	      	picoBlaze_pCodeOpCopy(PCOP2(PCI(pcr)->pcop)->pcopR), 0);
	      
	  }
	  
	  //DFPRINTF((stderr,"inserting pCode\n"));
	  picoBlaze_pCodeInsertAfter(pc, picoBlaze_newpCode(PCI(pcr)->op,pcop));
	} else if (pcr->type == PC_WILD) {
	  if(PCW(pcr)->invertBitSkipInst)
	    DFPRINTF((stderr,"We need to invert the bit skip instruction\n"));
	  picoBlaze_pCodeInsertAfter(pc,
			   pCodeInstructionCopy(PCI(peepBlock->target.wildpCodes[PCW(pcr)->id]),
						PCW(pcr)->invertBitSkipInst));
	} else if (pcr->type == PC_COMMENT) {
	  picoBlaze_pCodeInsertAfter(pc, picoBlaze_newpCodeCharP( ((pCodeComment *)(pcr))->comment));
	}
