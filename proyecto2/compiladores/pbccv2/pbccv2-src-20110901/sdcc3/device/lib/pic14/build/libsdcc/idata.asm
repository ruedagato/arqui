...
;[ICODE] idata.c:85: 	if iTemp24 [k37 lr32:34 so:0]{ ia0 a2p0 re0 rm0 nos1 ru0 dp0}{unsigned-int fixed}[r0x1052 r0x1053 ] == 0 goto _whilebreak_1($3)
	...
	BTFSC	STATUS,2
	GOTO	_00107_DS_
;[ICODE] idata.c:86: 	iTemp28 [k41 lr35:36 so:0]{ ia0 a2p0 re0 rm0 nos0 ru0 dp0}{const-char register}[r0x1052 ] = @[iTemp30 [k43 lr28:39 so:0]{ ia1 a2p0 re0 rm0 nos0 ru0 dp0}{const-char code* fixed}[r0x1050 r0x1051 ]]
;	.line	86; "idata.c"	*dst = *src;
	...
	BANKSEL	r0x100B
	MOVWF	r0x100B
;[ICODE] idata.c:86: 	*(iTemp32 [k45 lr29:39 so:0]{ ia1 a2p0 re0 rm0 nos0 ru0 dp0}{char near* fixed}[r0x104C r0x104D ]) := iTemp28 [k41 lr35:36 so:0]{ ia0 a2p0 re0 rm0 nos0 ru0 dp0}{const-char register}[r0x1052 ]
	MOVF	r0x1004,W
	MOVWF	FSR
	BCF	STATUS,7
	BTFSC	r0x1005,0
