...
/*
        VR - Began writing code to make PICOBLAZE C source files independent from
...
stats_t picoBlaze_statistics = { 0, 0, 0, 0 };

#define DEVICE_FILE_NAME    "picoBlazedevices.txt"

static PICOBLAZE_device default_device = {
    { "picoBlaze3", "picoBlaze3", "picoBlaze3", "picoBlaze3" }, /* name variants */
    0x600,
    0x80,
    { /* configuration words */
      0x300001, 0x30000d,
...
PICOBLAZE_device *picoBlaze = &default_device;
static PICOBLAZE_device *devices = NULL;
...
void picoBlaze_dump_equates(FILE *of, set *equs)
{
  regs *r;
...
void picoBlaze_dump_access(FILE *of, set *section)
{
  regs *r;
        ...
        for(; r; r = setNextItem(section)) {
                fprintf(of, "%s\tres\t%d\n", r->name, r->size);
                picoBlaze_statistics.adsize += r->size;
        }
}


int picoBlaze_regCompare(const void *a, const void *b)
{
...
int picoBlaze_symCompare(const void *a, const void *b)
{
...
void picoBlaze_dump_usection(FILE *of, set *section, int fix)
{
  ...
  regs *r1;
        ...
        qsort(rlist, i  /*elementsInSet(section)*/, sizeof(regs *), picoBlaze_regCompare);
...
#define EMIT_SINGLE_UDATA_SECTION       0
                ...
                for(r = setFirstItem(section); r; r = setNextItem(section)) {
                        fprintf(of, "%s\tres\t%d\n", r->name, r->size);
                        picoBlaze_statistics.udsize += r->size;
                }
#else
                for(r = setFirstItem(section); r; r = setNextItem(section)) {
                        //fprintf(of, "\nudata_%s_%s\tudata\n", moduleName, r->name);
...
void picoBlaze_dump_gsection(FILE *of, set *sections)
{
  regs *r;
  sectName *sname;
...
void picoBlaze_dump_isection(FILE *of, set *section, int fix)
{
  ...
  symbol **slist;
        ...
        /* sort symbols according to their address */
        qsort(slist, i, sizeof(symbol *), picoBlaze_symCompare);
        ...
        if(!fix) {
                ...
                for(s = setFirstItem(section); s; s = setNextItem(section)) {
                        ...
                        if(s->ival) {
                                ...
                                picoBlaze_printIval(s, s->type, s->ival, 'f', (void *)of);
                                picoBlaze_flushDB('f', (void *)of);
                        } else {
...
//                                      fprintf(stderr, "%s:%d printing code string from %s\n", __FILE__, __LINE__, s->rname);
                                        picoBlaze_pCodeConstString(s->rname , SPEC_CVAL (s->etype).v_char, getSize(s->type));
                                } else {
                                        assert(0);
                                }
                        }
...
void picoBlaze_dump_int_registers(FILE *of, set *section)
{
  regs *r, *rprev;
  int i;
  regs **rlist;
        ...
        /* sort symbols according to their address */
        qsort(rlist, elementsInSet(section), sizeof(regs *), picoBlaze_regCompare);
        ...
        for(r = setFirstItem(section); r; r = setNextItem(section)) {
                fprintf(of, "%s\tres\t%d\n", r->name, r->size);
                picoBlaze_statistics.intsize += r->size;
        }
 ...
 * Consider usind picoBlaze_find_device() instead!
 *
 * @param   name
 ...
 *      device, or NULL
 */
static PICOBLAZE_device *
find_in_list(const char *name, PICOBLAZE_device *head)
{
    int i;
 ...
 */
static void
picoBlaze_list_devices(PICOBLAZE_device *head)
{
    int i = 0;
...
get_line (FILE *file)
{
  ...
  if (dbuf_getline (&dbuf, file) != 0)
    {
      dbuf_chomp (&dbuf);
      /* (char *) type cast is an ugly hack since picoBlaze_find_device() modifies the buffer */
 ...
 */
static PICOBLAZE_device *
picoBlaze_find_device(const char *name)
{
    const char *path;
    ...
    const char *sep = " \t\n\r";
    FILE *f = NULL;
    PICOBLAZE_device *d = NULL, *template;
    PICOBLAZE_device *head = NULL, *tail = NULL;
	...
	printf("%s: searching device %s\n", __func__, name);

	if (!devices) {
        //printf("%s: ppppp searching %s\n", __func__, DEVICE_FILE_NAME);
        ...
        while (NULL != (line = get_line(f))) {
            ...
            if (!key) {
                ...
                if ((1 < res) || (3 > strlen(&buffer[3]))) {
                    SYNTAX("<name> (e.g., picoBlaze3) expected.");
                } else {
                    d = Safe_calloc(1, sizeof(PICOBLAZE_device));

                    // { "picoBlaze3", "picoBlaze3", "picoBlaze3", "picoBlaze3" }
                    buffer[0] = 'p';
                    buffer[1] = 'i';
                    buffer[2] = 'c';
 ...
 *
 *-----------------------------------------------------------------*/
void picoBlaze_init_pic(const char *pb_type_name)
{
    picoBlaze = picoBlaze_find_device(pb_type_name);

    if (&default_device == picoBlaze) {
        if (pb_type_name) {
        ...
        if (devices) {
            ...
            picoBlaze_list_devices(devices);
        } // if
        exit(EXIT_FAILURE);
    } // if
}
...
/*-----------------------------------------------------------------*
 *  char *picoBlaze_processor_base_name(void) - Include file is derived from this.
 *-----------------------------------------------------------------*/
char *picoBlaze_processor_base_name(void)
{

  if(!picoBlaze)
    return NULL;

  return picoBlaze->name[0];
}
...
int picoBlaze_checkAddSym(set **set, symbol *sym)
{
  symbol *tmp;
...
int picoBlaze_checkSym(set *set, symbol *sym)
{
  symbol *tmp;
...
/*-----------------------------------------------------------------*
 * void picoBlaze_groupRegistersInSection - add each register to its   *
 *      corresponding section                                      *
 *-----------------------------------------------------------------*/
void picoBlaze_groupRegistersInSection(set *regset)
{
  regs *reg;
  sectSym *ssym;
  int docontinue=0;
...
//                      fprintf(stderr, "%s:%d reg: %s\n", __FILE__, __LINE__, reg->name);
                        ...
                        if(reg->alias == 0x80) {
                                picoBlaze_checkAddReg(&picoBlaze_equ_data, reg);
                        } else
                        if(reg->isFixed) {
                                picoBlaze_checkAddReg(&picoBlaze_fix_udata, reg);
                        } else
                        if(!reg->isFixed) {
                                if(reg->pc_type == PO_GPR_TEMP)
                                        picoBlaze_checkAddReg(&picoBlaze_int_regs, reg);
                                else {
                                        if(reg->accessBank) {
                                                if(reg->alias != 0x40)
                                                        picoBlaze_checkAddReg(&picoBlaze_acs_udata, reg);
                                        } else
...
/*-----------------------------------------------------------------*
 *  void picoBlaze_assignConfigWordValue(int address, int value)
 *
...
void picoBlaze_assignConfigWordValue(int address, unsigned int value)
{
  int i;
        ...
        for(i=0;i<picoBlaze->cwInfo.confAddrEnd-picoBlaze->cwInfo.confAddrStart+1;i++) {
                if((address == picoBlaze->cwInfo.confAddrStart+i)
                  && (picoBlaze->cwInfo.crInfo[i].mask != -1)
                  && (picoBlaze->cwInfo.crInfo[i].mask != 0)) {

                        picoBlaze->cwInfo.crInfo[i].value = value;
                        picoBlaze->cwInfo.crInfo[i].emit = 1;
                        return;
                }
        }
}
...
void picoBlaze_assignIdByteValue(int address, char value)
{
  int i;
        ...
        for(i=0;i<picoBlaze->idInfo.idAddrEnd-picoBlaze->idInfo.idAddrStart+1;i++) {
                if(address == picoBlaze->idInfo.idAddrStart+i) {
                        picoBlaze->idInfo.irInfo[i].value = value;
                        picoBlaze->idInfo.irInfo[i].emit = 1;
                }
        }
}
